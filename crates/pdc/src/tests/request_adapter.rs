...
fn classify_paths_uses_profile_whitelist_and_diagnostic_parser_categories() {
    ...
    let input = frames([
        ...
        json!({"jsonrpc":"2.0","id":2,"method":"pdc/classifyPaths","params":{"paths":["events/test.txt","localisation/test_l_english.yml","ThirdPartyLicenses.txt","interface/test.gui"]}}),
...
fn mission_preview_returns_renderer_ready_tree_data() {
    ...
    let input = frames([
        ...
        json!({"jsonrpc":"2.0","id":2,"method":"pdc/missionPreview","params":{"path":"missions/test.txt","uri":format!("{root_uri}/missions/test.txt"),"version":7,"text":"main_tree = {\n\tslot = 1\n\ta1 = { position = 1 icon = mission_alpha }\n\ta2 = { position = 2 required_missions = { a1 } }\n}\n\nbranch_tree = {\n\tslot = 2\n\tb1 = { position = 1 required_missions = { external_id } }\n}\n"}}),
        json!({"jsonrpc":"2.0","id":3,"method":"shutdown","params":{}}),
        json!({"jsonrpc":"2.0","method":"exit"}),
    ]);
    let mut output = Vec::new();
    ...
    assert!(a2["sourceRange"]["end"]["character"].is_u64());
    ...
    assert!(groups.iter().all(|group| {
        group["sourceRange"]["start"]["line"].is_u64()
            && group["sourceRange"]["end"]["character"].is_u64()
    }));
...
fn workspace_diagnostics_batches_indexed_disk_files_without_opening_overlays() {
    ...
    let input = frames([
        json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"workspaceFolders":[{"uri":root_uri,"name":"test"}],"capabilities":{}}}),
        json!({"jsonrpc":"2.0","method":"initialized","params":{}}),
        json!({"jsonrpc":"2.0","id":2,"method":"pdc/workspaceDiagnostics","params":{"offset":0,"limit":1}}),
        json!({"jsonrpc":"2.0","id":3,"method":"pdc/workspaceDiagnostics","params":{"offset":1,"limit":1}}),
...
#[test]
fn workspace_files_exposes_active_source_roots_without_contents() {
    let (root, root_uri) = temp_workspace_dir();
    let events = root.join("events");
    fs::create_dir_all(&events).expect("events directory");
    fs::write(
        events.join("loaded.txt"),
        "country_event = { id = loaded.1 }\n",
    )
    .expect("source file");
    let input = frames([
        json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"workspaceFolders":[{"uri":root_uri,"name":"test"}],"capabilities":{}}}),
        json!({"jsonrpc":"2.0","method":"initialized","params":{}}),
        json!({"jsonrpc":"2.0","id":2,"method":"pdc/workspaceFiles"}),
        json!({"jsonrpc":"2.0","id":3,"method":"shutdown","params":{}}),
        json!({"jsonrpc":"2.0","method":"exit"}),
    ]);
    let mut output = Vec::new();
    ...
    let loaded = files
        .iter()
        .find(|file| file["logicalPath"] == "events/loaded.txt")
        .expect("loaded source file");
    assert!(loaded["active"].as_bool().unwrap_or(false));
    assert!(
        loaded["uri"]
            .as_str()
    ...
    assert!(
        loaded.get("text").is_none(),
        "workspace files must not return contents"
    );
    fs::remove_dir_all(root).expect("cleanup");
}
...
fn noncanonical_document_uri_preserves_rule_path_context() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let container = std::env::temp_dir().join(format!("pdc-path-context-{nonce}"));
...
fn initialize_reports_stages_via_log_message_and_work_done_progress() {
    ...
    assert!(
        logs.iter().any(|log| {
            log["params"]["message"]
                .as_str()
                .is_some_and(|message| message.contains("pdc initializing"))
        }),
    ...
    assert_eq!(begin["params"]["value"]["message"], "Starting pdc…");
