...
fn builtin_rule_has_key(snapshot: &AnalysisSnapshot, context: &str, key: &str) -> bool {
    snapshot
        ...
        .any(|rule| {
            matches!(
                &rule.key,
                rules::KeyMatcher::Exact(expected)
...
fn validate_semantic_container(
    ...
    for property in properties {
        ...
        let mut transition_rule: Option<&rules::SemanticRule> = None;
        let mut transition_resolved = false;
        let destination = if let Some(fact) = cached_child_fact {
            Some((
                engine::intern_shard_string(&fact.context),
                fact.parent_path
                    .iter()
                ...
                .is_dynamic_scope_expression(&property.key)
        {
            ...
            next_scope.current = engine::intern_shard_string("any");
            Some((
                engine::intern_shard_string(context),
                parent_path.to_vec(),
                next_scope,
            ))
        } else {
            ...
            let numeric_overflow = value.parse::<f64>().is_ok()
                && rules.iter().any(|rule| {
                    matches!(rule.shape, RuleShape::LeafValue)
                        && matches!(
                            &rule.value,
                            rules::ValueMatcher::Int { .. } | rules::ValueMatcher::Float { .. }
                        )
                });
