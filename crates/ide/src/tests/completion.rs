...
fn workspace_member_index_tracks_overlay_open_and_close() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-member-overlay-{nonce}"));
...
fn on_action_event_block_completion_excludes_namespace_headers() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-event-symbols-{nonce}"));
...
fn leaf_value_container_completion_offers_typed_workspace_members() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-mission-complete-{nonce}"));
...
fn leaf_value_clause_bare_value_completion_offers_typed_workspace_members() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-mission-bare-{nonce}"));
    fs::create_dir_all(root.join("missions")).expect("mission directory");
    let path = root.join("missions/test.txt");
    let text = concat!(
        "test_series = { slot = 1 generic = no ai = yes has_country_shield = no\n",
...
fn scripted_definition_completion_snippet_includes_parameters() {
    ...
    let root = std::env::temp_dir().join(format!("ide-snippet-{}", std::process::id()));
...
fn dynamic_call_blocks_complete_only_the_owners_parameter_keys() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-call-keys-{nonce}"));
    let definitions = root.join("common/scripted_effects");
...
fn dynamic_argument_values_follow_direct_and_nested_body_constraints() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-call-values-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_complete.txt"),
...
fn dynamic_affixed_value_arguments_complete_stripped_members() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-affixed-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn dynamic_bare_parameter_infers_quoted_effect_completion_context() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-quoted-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
...
fn dynamic_argument_value_inference_handles_conditionals_scope_and_conflicts() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-value-kinds-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_complete.txt"),
...
fn vanilla_cache_only_dynamic_value_completion_uses_persisted_body_constraints() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-cached-completion-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn non_enumerable_dynamic_value_constraints_suppress_generic_fallback() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-unenumerable-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn vanilla_cache_only_dynamic_completes_inside_quoted_effect_payload() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-cached-quoted-complete-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_cached.txt"),
...
fn vanilla_cache_dynamic_templates_preserve_nested_conditional_and_scope_semantics() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-cached-dynamic-semantics-{nonce}"));
    let definitions = root.join("common/scripted_effects");
...
fn current_mod_dynamic_template_overrides_cached_vanilla_template() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-priority-{nonce}"));
...
fn closed_flag_kinds_complete_indexed_overlay_and_engine_seeded_names() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-flag-completion-{nonce}"));
...
fn dynamic_definition_completion_filters_by_entry_contract_at_call_sites() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-contract-filter-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_complete.txt"),
...
fn dynamic_trigger_completion_filters_by_entry_contract() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-trigger-filter-{nonce}"));
    let triggers = root.join("common/scripted_triggers");
...
fn dynamic_definition_completion_keeps_all_contracts_under_unknown_scope() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-unknown-scope-{nonce}"));
...
fn dynamic_key_position_parameter_completes_command_names() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-key-cmd-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_complete.txt"),
...
fn dynamic_affixed_key_parameter_completes_stripped_key_members() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-key-affix-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_complete.txt"),
...
fn dynamic_key_position_parameter_respects_site_scope() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-key-scope-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_complete.txt"),
...
fn dynamic_body_completion_seeds_scope_from_own_contract() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-body-seed-{nonce}"));
    let definitions = root.join("common/scripted_effects/00_defs.txt");
    let path = root.join("common/scripted_effects/00_open.txt");
    std::fs::create_dir_all(definitions.parent().expect("parent")).expect("definition directory");
    std::fs::write(
...
fn dynamic_trigger_body_completion_seeds_scope_from_own_contract() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-trigger-seed-{nonce}"));
...
fn luck_root_completion_offers_workspace_country_tags() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-luck-completion-{nonce}"));
