...
fn scripted_localisation_names_feed_indexed_diagnostics_and_completion() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-scripted-loc-{nonce}"));
...
fn dynamic_bare_parameter_validates_quoted_effect_payload_at_call_site() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-quoted-diag-{nonce}"));
...
fn quoted_payload_host(
    nonce: &str,
    definitions: &str,
) -> (engine::AnalysisHost, std::path::PathBuf) {
    let root = std::env::temp_dir().join(format!("ide-quoted-payload-{nonce}"));
...
fn affixed_value_host(
    ...
    let root = std::env::temp_dir().join(format!("ide-affixed-value-{nonce}"));
...
fn dynamic_definitions_preserve_literal_quoted_script_through_nested_calls() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-nested-quoted-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn dynamic_definitions_omit_missing_optional_forwarded_arguments() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-forwarded-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_validate.txt"),
...
fn runtime_branch_dynamic_accepts_the_amount_only_legitimacy_call() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-legitimacy-branch-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn cached_runtime_branch_dynamic_recomputes_optional_parameters_from_the_template() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-cached-legitimacy-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_legitimacy.txt"),
...
fn dynamic_calls_validate_required_and_duplicate_parameters() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-args-{nonce}"));
    let definitions = root.join("common/scripted_effects");
...
fn dynamic_invocation_diagnostics_report_precise_messages() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-messages-{nonce}"));
...
fn dynamic_rule_call_site_validates_argument_values_and_dispatch_keys() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-expand-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn dynamic_rule_dispatch_accepts_known_keys_and_scope_registers() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-dispatch-ok-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn dynamic_definitions_defer_parameterized_nested_invocations() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-forward-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
...
fn dynamic_missing_required_parameter_is_reported_once() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-required-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn all_optional_dynamic_definition_accepts_scalar_invocation() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-optional-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
...
fn dynamic_rule_arguments_reject_block_bindings_and_use_last_duplicate_scalar() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-binding-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn dynamic_definitions_activate_conditionals_and_report_cycles() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-cycle-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
        definitions.join("00_expand.txt"),
...
fn deeply_nested_dynamic_rule_chains_validate_without_expansion_budgets() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-depth-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
...
fn vanilla_cache_only_dynamic_row_records_unknown_body_statement() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-cached-dynamic-{nonce}"));
    let definitions = root.join("common/scripted_effects");
    std::fs::create_dir_all(&definitions).expect("definition directory");
    std::fs::write(
...
fn vanilla_cache_only_dynamic_validates_quoted_payload_at_exact_call_site_range() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-cached-quoted-diag-{nonce}"));
...
fn parameterized_flag_writes_admit_their_expansions() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-flag-patterns-{nonce}"));
    let effects = root.join("common/scripted_effects");
...
fn dispatch_keys_inside_limits_validate_in_the_trigger_context() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-limit-dispatch-{nonce}"));
    let effects = root.join("common/scripted_effects");
    std::fs::create_dir_all(&effects).expect("scripted effects directory");
...
fn tooltip_blocks_do_not_dispatch_validate() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-tooltip-dispatch-{nonce}"));
    let effects = root.join("common/scripted_effects");
    std::fs::create_dir_all(&effects).expect("scripted effects directory");
...
fn value_keyed_branch_containers_do_not_dispatch_validate() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-branch-dispatch-{nonce}"));
    let effects = root.join("common/scripted_effects");
    std::fs::create_dir_all(&effects).expect("scripted effects directory");
...
fn embedded_modifier_templates_do_not_constrain_the_argument() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-embedded-modifier-{nonce}"));
    let effects = root.join("common/scripted_effects");
    std::fs::create_dir_all(&effects).expect("scripted effects directory");
...
fn nested_government_mechanic_powers_feed_dynamic_value_validation() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-government-powers-{nonce}"));
    let mechanics = root.join("common/government_mechanics");
    std::fs::create_dir_all(&mechanics).expect("mechanics directory");
    std::fs::write(
        mechanics.join("00_test.txt"),
...
fn evicted_frontend_diagnostics_match_retained() {
    let nonce = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("pdc-evict-debug-{nonce}"));
...
fn dynamic_cycles_are_reported_at_definition_sites() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-cycles-{nonce}"));
    let effects = root.join("common/scripted_effects");
    std::fs::create_dir_all(&effects).expect("scripted effects directory");
    std::fs::write(
...
fn modifier_scope_mismatch_reports_cross_scope_modifier_applications() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-modifier-scope-{nonce}"));
...
fn dynamic_call_sites_are_validated_against_entry_contracts() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-call-sites-{nonce}"));
...
fn overlay_without_physical_path_routes_dynamic_definition_directories() {
    ...
    let hover = hover(&snapshot, &definitions, text::TextSize::from(0u32)).expect("hover");
    assert!(
...
fn luck_test_host(nonce: u128) -> AnalysisHost {
    let root = std::env::temp_dir().join(format!("ide-luck-{nonce}"));
