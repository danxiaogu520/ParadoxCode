...
状态：`implemented, acceptance reopened`（2026-07-20）；查询功能和 LSP integration 已实现，query-time 全 workspace 重解析、深拷贝 snapshot、同步 diagnostics 和 event-loop 上的语言查询已消除；parse/lower 事件循环阻塞与 analysis 内部细粒度取消仍不满足已接受边界。
...
7. 将 LSP transport 迁移到类型化协议层，增加 worker、debounce、版本门和在途取消（stdio reader 分离、semantic diagnostics 200ms debounce/版本门、snapshot request worker 与在途结果取消已完成；类型化协议、parse worker 和 analysis 内部 cancellation checkpoint 待完成）；
