    ...
    let input = ScriptedReader::new([
        ...
        (
            json!({
                ...
                "params": {"command": "pdc/reindexWorkspace", "arguments": []}
            }),
            None,
        ),
        (
...
fn ready_pass_publishes_when_scan_completes_after_shutdown() {
    ...
    let ready_at = responses
        ...
        .expect("pdc/ready notification after the initial scan");
    assert!(
    ...
    let scan_end_at = responses
        ...
        .position(|value| {
            value["method"] == "$/progress"
                && value["params"]["token"]
                    .as_str()
                    .is_some_and(|token| token.starts_with("pdc-scan-"))
...
fn exhausted_initial_scan_retries_still_reach_ready() {
    ...
    assert!(
        responses.iter().any(|value| value["method"] == "pdc/ready"),
