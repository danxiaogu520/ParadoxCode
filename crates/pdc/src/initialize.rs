...
/// [`rules::RuleSet`] directly and no user-controlled path can replace it.
...
pub(crate) fn prepare_initialize_candidate(
    ...
    if let Some(log) = callbacks.log {
        log(&format!(
            "pdc initializing (game profile '{game_id}', rules hash {rule_hash})"
        ));
    }
    ...
    // `pdc/ready` is emitted once the scan and any cache installs finish.
