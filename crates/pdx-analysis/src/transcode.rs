...
pub(crate) fn transcode_diagnostics(
    ...
    let counts = pdx_codec::classify_text_counts(source);
    ...
    match pdx_codec::classify_text(source) {
        pdx_codec::Classification::Escaped | pdx_codec::Classification::Mixed
            if escaped_with_damage =>
        {
            let decoded = pdx_codec::decode_text(source);
            ...
            if input.format == FileFormat::Script
                ...
                && !pdx_codec::script_roundtrip_is_canonical(source)
            {
                diagnostics.push(Diagnostic::new(
        ...
        pdx_codec::Classification::Mixed => {
        ...
        pdx_codec::Classification::Escaped => {
            if input.format == FileFormat::Script
                && !pdx_codec::script_roundtrip_is_canonical(source)
            {
                diagnostics.push(Diagnostic::new(
        ...
        pdx_codec::Classification::Readable => {
        ...
        pdx_codec::Classification::Ascii => {}
    }
...
fn unencodable_diagnostics(
    ...
    let profile = match input.format {
        FileFormat::Localisation => pdx_codec::Profile::Localisation,
        FileFormat::Script => pdx_codec::Profile::Script,
    };
    let mut diagnostics = Vec::new();
    for (offset, character) in source.char_indices() {
        cancellation.checkpoint()?;
        if let Some(kind) = pdx_codec::file_unencodable_kind(u32::from(character), profile) {
            if diagnostics.len() >= MAX_TRANSCODE_DIAGNOSTICS {
                break;
            }
            let reason = match kind {
                pdx_codec::UnencodableKind::MangledLowPlane => {
                    "U+0100..U+0FFF characters are silently mangled by the EU4 transcoder"
                }
                pdx_codec::UnencodableKind::BeyondBmp => {
...
fn first_raw_cjk(source: &str) -> Option<TextRange> {
    source
        .char_indices()
        .find(|(_, character)| pdx_codec::is_raw_cjk(u32::from(*character)))
...
fn first_mixed_evidence(source: &str) -> Option<TextRange> {
    source
        ...
        .find(|(_, character)| {
            ...
            (0x10..=0x13).contains(&code_point) || pdx_codec::is_raw_cjk(code_point)
        })
        .and_then(|(offset, _)| char_range(source, offset))
}
