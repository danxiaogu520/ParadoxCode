...
fn symbol_hover_explains_active_and_shadowed_source_roots() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-hover-sources-{nonce}"));
...
fn localisation_values_by_key_uses_index_priority_and_english_preference() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-localisation-values-{nonce}"));
    let vanilla = root.join("vanilla");
    let current = root.join("current");
...
fn localisation_values_by_key_apply_the_layer_then_read_order_total_order() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-localisation-order-{nonce}"));
    let vanilla = root.join("vanilla");
...
fn vanilla_cache_localisation_hover_shows_derived_text_without_source_state() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-vanilla-hover-{nonce}"));
    let vanilla = root.join("vanilla");
    let current = root.join("current");
    ...
    let mut vanilla_host = eu4_host(game::eu4::bootstrap_rules());
    vanilla_host.apply_change(WorkspaceChange::SetSourceRoots(vec![SourceRoot::new(
        SourceRootId::new(0),
        SourceRootKind::Vanilla,
        vanilla.clone(),
...
fn dynamic_parameter_hovers_and_payload_arguments_are_diagnosable() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-dynamic-hover-{nonce}"));
...
fn affixed_value_parameter_hover_names_the_render_and_expected_domain() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-affixed-hover-{nonce}"));
    let effects = root.join("common/scripted_effects");
    fs::create_dir_all(&effects).expect("scripted effects directory");
...
fn dynamic_parameter_hover_replays_bindings_aware_sites() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-hover-replay-{nonce}"));
    let effects = root.join("common/scripted_effects");
    fs::create_dir_all(&effects).expect("scripted effects directory");
    let definitions_body = concat!(
        "pick = {\n",
...
fn event_hover_falls_back_to_the_generated_title_key() {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("ide-event-title-{nonce}"));
