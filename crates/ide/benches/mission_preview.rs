...
//!
//! The `pdc/missionPreview` handler resolves every `{mission}_title` key through
//! `ide::localisation_values_by_key` on each preview refresh (the VS Code
...
use game::eu4;
use ide::{CancellationToken, localisation_values_by_key};
...
fn dense_files() -> usize {
    std::env::var("PDC_BENCH_PREVIEW_FILES")
        .ok()
...
fn dense_entries() -> usize {
    std::env::var("PDC_BENCH_PREVIEW_ENTRIES")
        .ok()
        .and_then(|value| value.parse().ok())
        .filter(|count| *count > 0)
...
fn title_count() -> usize {
    std::env::var("PDC_BENCH_PREVIEW_TITLES")
        .ok()
        .and_then(|value| value.parse().ok())
        .filter(|count| *count > 0)
