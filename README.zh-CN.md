...
- EU4 profile 会从支持的 scripted localisation 目录拼写中发现定义，将 `name` 字段作为
  `defined_text` 编入索引，并支持可感知未保存覆盖层的 `$NAME$` 悬停查询。本地化文档仍参与
  索引与导航，但不会发布 LSP 诊断或补全项，避免在正文编辑时产生干扰。
...
**最新版本：v0.3.3**（2026-09-12）。EU4 分析与索引功能已实现、测试，并通过标签驱动的发布流水线发布（见[发布](#发布)）。本版本按 rust-analyzer 的分层重构了仓库（十二个 crate，语言服务器二进制更名为 `paradoxcode`，用户可见契约去掉 `pdx` 前缀），用 `cargo tools gates` 取代了 shell 工具层，修复了打开文档时整仓校验占满全部工作核心的问题，通过模板键匹配器让引擎参数化的修正器族真正参与校验，并端到端支持 UNC 路径。0.x 仍在早期成熟期，欢迎早期使用者通过 issue 模板反馈问题，以便在下一个版本中修复。
