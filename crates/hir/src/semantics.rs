...
pub(super) fn lower_semantics(
    ...
    let enum_localisation_rules = rules
        ...
        .iter()
        .filter(|rule| {
            matches!(rule.key, rules::KeyMatcher::Enum(_))
    ...
    match rule.shape {
        ...
        RuleShape::Node | RuleShape::ValueClause => {
            property.value_range.is_some()
                && matches!(
                    rule.value,
                    rules::ValueMatcher::AnyScalar | rules::ValueMatcher::Opaque(_)
                )
        }
    }
}
...
fn semantic_localisation_reference(
    ...
    root_contexts: &mut RootContextCache,
    rules: &RuleSet,
    enum_localisation_rules: &[&rules::SemanticRule],
) -> Option<HirReference> {
    ...
    let matches_rule = |rule: &rules::SemanticRule| {
        (context.is_some_and(|context| rule.context.eq_ignore_ascii_case(context))
            || root_context.as_deref().is_some_and(|context| {
                rule.context.eq_ignore_ascii_case(context)
    ...
    expected
        ...
        .all(|(expected, actual)| {
            if expected.starts_with('<') && expected.ends_with('>') {
                ...
                rules::ValueMatcher::Date.matches(actual, |_, _| false, |_, _| false, |_, _| false)
...
fn semantic_localisation_key_matches(
    rules: &RuleSet,
    matcher: &rules::KeyMatcher,
    key: &str,
) -> bool {
    match matcher {
        rules::KeyMatcher::Exact(expected) => expected.eq_ignore_ascii_case(key),
        rules::KeyMatcher::Enum(name) => rules
            .model()
            .semantic
            .enum_values
            .get(name)
