...
impl SyntheticWorkspace {
    fn create(file_count: usize) -> Self {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("engine-benchmark-{}-{nonce}", std::process::id()));
    ...
    fn event_path(&self, index: usize) -> PathBuf {
        self.root
            .join("events")
            .join(format!("event-{index:05}.txt"))
    }
}
...
fn main() {
    ...
    let (single_disk_change, report) = measured(|| {
        host.refresh_source_roots()
            .expect("single-file disk refresh")
    });
    assert_eq!(report.indexed_files, count);
    black_box(host.snapshot());
    ...
    println!(
        "initial scan/index:     {:>10.3} ms",
        display_millis(initial_scan)
    );
    println!(
        "unchanged full refresh: {:>10.3} ms",
        display_millis(unchanged_scan)
    );
    println!(
        "one disk-file refresh:  {:>10.3} ms",
        display_millis(single_disk_change)
    );
    println!(
        "one overlay edit:        {:>10.3} ms",
        display_millis(single_overlay_edit)
    );
}
