...
Eu4Rules
  ...
  resolution_policies
  localisation_descriptors / type-instance localisation bindings
...
schema 16 的 metadata 必须包含 `game_id = "eu4"`、`schema_version` 和 `rule_hash`；artifact 同时保存带 subtype condition 的 type-instance localisation bindings 及 semantic rule 的 deprecated 标志。`game_id` 进入 canonical hash；缺失或与所选 `Eu4Profile` 不一致时，server 在启动阶段拒绝 artifact，不能降级后继续提供带错误语义的服务。
