...
fn walk_cst(node: parser::CstNode<'_>, stats: &mut (usize, usize)) {
    stats.0 += 1;
...
fn main() {
    ...
    if let Some(appdata) = std::env::var_os("LOCALAPPDATA") {
        let cache_path =
            std::path::Path::new(&appdata).join("ParadoxCode/cache/eu4/vanilla.pdcindex");
        match engine::IndexCache::load(&cache_path) {
            Ok(cache) => {
    ...
    // vanilla files (cache-installed, source not retained).
    {
        ...
        let entry_bytes = definition_entries * std::mem::size_of::<engine::Definition>()
            + reference_entries * std::mem::size_of::<engine::Reference>();
        println!(
        ...
        let entry = std::mem::size_of::<(text::TextRange, text::PositionRange)>();
        println!(
        ...
        println!(
            ...
            mib((slack_entries * std::mem::size_of::<engine::Reference>()) as f64)
        );
    }
    ...
    for file in snapshot.source_files().values() {
        ...
        let format = match category.parser {
            rules::ParserKind::Script => parser::FileFormat::Script,
            rules::ParserKind::Localisation => parser::FileFormat::Localisation,
            _ => continue,
        };
    ...
    {
        let snapshot = host.snapshot();
        let cancellation = ide::CancellationToken::new();
        ...
        for file in &sorted_files {
            ...
            if let Ok(diagnostics) =
                ide::source_file_diagnostics_with_cancellation(&snapshot, file.id, &cancellation)
            {
