...
use engine::intern_shard_string;
use engine::{
...
use rules::{GameProfile, KeyMatcher, RuleShape, ValueMatcher};
...
pub(crate) struct CachedScopeFactInput<'data, 'hir> {
    ...
    pub(crate) matching: &'data [&'hir rules::SemanticRule],
...
pub(crate) struct SemanticTransitionInput<'data, 'rule> {
    pub(crate) snapshot: &'data AnalysisSnapshot,
    pub(crate) matching: &'data [&'rule rules::SemanticRule],
    pub(crate) selected_alternative: Option<&'data str>,
    pub(crate) context: &'data str,
    pub(crate) parent_path: &'data [Arc<str>],
    pub(crate) property: &'data ScriptProperty,
...
) -> Option<&'rule rules::SemanticRule> {
...
pub(crate) fn semantic_transition_candidates<'rule>(
    matching: &[&'rule rules::SemanticRule],
    selected_alternative: Option<&str>,
    property: &ScriptProperty,
    scope: &ScopeContext,
) -> Vec<&'rule rules::SemanticRule> {
...
pub(crate) fn semantic_transitions_equivalent(rules: &[&rules::SemanticRule]) -> bool {
...
pub(crate) fn semantic_selected_alternative(
    ...
    let parent_ok = |rule: &rules::SemanticRule| {
        semantic_parent_path_matches(snapshot, &rule.parent_path, parent_path)
    };
...
pub(crate) fn semantic_rule_severity<'a>(
    rules: impl IntoIterator<Item = &'a rules::SemanticRule>,
...
pub(crate) fn typed_prefix_operand_allows(
    ...
    match operand {
        rules::TypedPrefixOperand::NumericOrBool => {
            matches!(
                matcher,
...
fn membership_view_type_names(rules: &rules::RuleSet) -> Vec<String> {
    let mut names = vec![
        "variable".to_owned(),
