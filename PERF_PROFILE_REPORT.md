...
## 十二、2026-09-01 第三轮:M1 驱逐 / V3 降低 / M2 内存 / V4 memo(六提交)

语料与门槛不变(3047072888,7,907 文件;诊断 digest `0xd8189a9fdfc02166`、count=12016,
每次提交后复验一致)。全部提交过 pre-commit 全量门(fmt/clippy -D warnings/测试)。

### 12.1 提交与内容

| 提交 | 内容 | 直接效果(mem_probe / LSP 口径) |
|---|---|---|
| `7190e3a9` M1 | 扫描期即驱逐关闭文件前端(CST/HIR),位置表先行提取;验证期按需重 lowering | 峰值窗不再常驻 CST/HIR |
| `aa9106c` V3 | scope-fact 降低的规则键匹配分桶(全字面量哈希桶 + 动态段按长度),`ChildMatchMemo` 跨兄弟复用 | lowering 11.0 → 8.5s |
| `31a6069` M2a | 索引向量精确收缩(`shrink_to_fit`)消除 ~102MiB 松弛;删除 FileState 重复位置缓存(-46MiB) | 探针稳态 0.810 / 峰值 0.906GiB |
| `8d789b5` | LSP 验证完成日志恒定输出(修复 run-triple 等待条件) | 测量口径修复 |
| `206810f` V4 | `workspace_member` 按快照 revision 线程本地 memo(嵌套 FxHashMap,借用探针零分配,上限 2^16) | 诊断 pass 38.2 → 35.0s;LSP 70.5 → 67.0 CPU-s |
| `e3566cd` M2b | 缓存安装期本地化预览按"偏好语言 ∪ english"保留(定义仍全量索引;设计变更见提交说明);预览向量精确收缩 | LSP peakWS 1.07 → 0.96GiB;预览活负载 61 → 15MiB |

### 12.2 内存归因(探针,mem8 口径)

稳态构成(0.79GiB WS / 活数据):源文本 62MiB、shard 符号 206MiB、位置表 ~234MiB(索引)
+88MiB(状态)、预览 15MiB(原 61MiB)、规则/记录 ~14MiB。`drop(host)` 消融后 0.18GiB,
证明驻留为真实活数据。M2b 的 90MiB 释放中仅 ~28MiB 反映为 WS 下降:安装期仍先物化全量
预览再过滤,且释放页在复用前保留于 WS;LSP 口径峰值因验证期并发瞬态更敏感,实测 -110MiB。

勘误:mem6 起"scan wall 13.8s"为探针新增 PHASE 睡眠(2×4s)计入所致,扫描实际未回归。

### 12.3 最终对标(无空闲窗,同机连续三臂)

| 指标 | CWTools F# | cwtools-rs | pdx-ls(本轮后) | pdx / cwtools-rs | 本会话起点 |
|---|---|---|---|---|---|
| wall | 49.0s | 4.0s | **18.0s** | 4.5× | 47.1s |
| CPU | 149.0 CPU-s | 14.5 CPU-s | **62.9 CPU-s** | **4.3×**(原 13.6×) | 196.4 CPU-s |
| 峰值内存 | 4.73 GiB | 0.33 GiB | **0.95 GiB** | **2.9×**(原 9.4×) | 2.83 GiB |
| 稳态内存 | 3.41 GiB | 0.27 GiB | **0.87 GiB** | 3.2× | — |

单线程诊断 pass:157.3s(原始)→ **35.0s**(-78%)。全工作区验证 CPU 本会话累计 **-68%**。

### 12.4 距"全面严格低于 cwtools-rs"的剩余路径(未实施)

1. **CPU(-30~35% 潜力)**:扫描期验证融合——M1 驱逐后验证期重 lowering 占验证 CPU
   ~24%,与扫描期解析/降低完全重复;融合为"扫描时验证后驱逐"可消除。属架构级改动,
   需 digest 门内分阶段落地。
2. **内存(-~300MiB 潜力)**:位置表按需化(~234MiB,hover/导航才需要,可磁盘缓存侧
   按文件懒加载);`Reference` 结构紧凑化(206MiB shard 符号,ranges 打包/符号 id 内联);
   源文本 Arc 去重。
3. **分配器(~20% CPU)**:降低/扫描路径 arena 化(ntdll 堆调用 6.2% + free 4.9% +
   vcruntime 7.5%),与 1 协同设计。
4. wall 受脚本固定段约束;真实编辑延迟(open→diag p50 277ms)不变,与对标口径无关。

结论:六提交后 CPU/内存/启动全部大幅收敛且行为门(诊断 digest、523 项测试)逐次保持;
"严格低于 cwtools-rs"尚需 12.4 的架构级三项,建议作为下一轮独立立项。
