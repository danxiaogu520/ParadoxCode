...
1. 接受 RFC 0013，拆分通用规则 runtime 与 EU4 profile，迁移期间保留兼容 re-export（`pdx-rules`、`pdx-game-eu4`、schema 12 `game_id` 校验与 `pdx-eu4` facade 已建立；data-only profile 已显式贯穿 CLI/LSP/host/snapshot，event、scripted effect/trigger、localisation 的 definition/reference 已迁移，其他 EU4 特殊解释仍待迁移）；
