...
fn background_reindex_options_are_bounded_and_default_to_opt_in() {
    ...
    let defaults = resolve_source_roots(Some(&root), None, &engine::WorkspaceScanToken::new())
    ...
    assert_eq!(
        ...
        Some(&Some(ide::Severity::Warning))
    );
    assert_eq!(
        advanced
...
fn editor_options_load_ordered_dependencies_and_keep_them_read_only() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("pdc-editor-options-{nonce}"));
    ...
    let vanilla_cache_path = cache_dir.join("vanilla.pdcindex");
    vanilla_cache
    ...
    assert_eq!(roots[0].kind, engine::SourceRootKind::Vanilla);
    assert_eq!(roots[0].order, 0);
    assert_eq!(roots[1].kind, engine::SourceRootKind::Dependency);
...
fn initialize_defers_an_existing_vanilla_cache() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let container = std::env::temp_dir().join(format!("pdc-deferred-cache-{nonce}"));
...
fn stale_cache_regeneration_reports_work_done_progress() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let container = std::env::temp_dir().join(format!("pdc-regen-progress-{nonce}"));
    let cache_path = stale_cache_fixture(&container);
...
fn valid_cache_load_reports_work_done_progress() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let container = std::env::temp_dir().join(format!("pdc-valid-cache-progress-{nonce}"));
    ...
    let ready_index = responses
        ...
        .position(|value| {
            value["method"] == "window/logMessage"
                && value["params"]["message"]
                    .as_str()
                    .is_some_and(|message| message.contains("pdc ready"))
        })
        .expect("ready log");
    let ready_notification_index = responses
        .iter()
        .position(|value| value["method"] == "pdc/ready")
...
fn stale_vanilla_cache_reports_regeneration_failure_explicitly() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let container = std::env::temp_dir().join(format!("pdc-regen-failure-{nonce}"));
    let cache_path = stale_cache_fixture(&container);
...
fn unavailable_explicit_cache_is_rebuilt_from_discovered_source() {
    ...
    let automatic = AutoVanillaConfiguration {
        descriptor: game::eu4::INSTALL_DESCRIPTOR,
    ...
    let (cache, message) = run_index_cache_load_with_options(
        IndexCacheLoadRequest {
            ...
            log: None,
            progress: None,
            scan_limits: engine::WorkspaceScanLimits::default(),
...
fn unavailable_configured_cache_is_rebuilt_from_configured_source() {
    ...
    let cache_path = root.join("user/cache/eu4/vanilla.pdcindex");
...
fn explicit_cache_precedes_user_discovery_configuration() {
    ...
    game.vanilla_cache = Some(root.join("user/cache/eu4/vanilla.pdcindex"));
    configuration
        .save(&automatic.user_paths.config_file)
        .expect("save user configuration");

    let explicit_cache = root.join("cache/vanilla.pdcindex");
    let mut resolved = ResolvedSourceRoots {
        ...
        scan_filters: engine::WorkspaceScanFilters::default(),
    ...
    let resolved = super::resolve_source_roots(
        ...
        Some(json!({
            ...
            "dependencies": [
                ...
                {"id": "cached-dep", "path": "deps/cached", "index": "cache/cached-dep.pdcindex"}
            ]
        })),
        &engine::WorkspaceScanToken::new(),
    )
...
fn existing_dependency_index_cache_is_installed_in_the_background() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let container = std::env::temp_dir().join(format!("pdc-dep-cache-load-{nonce}"));
    let workspace = container.join("workspace");
    ...
    let cache_path = container.join("dependency.pdcindex");
...
fn missing_dependency_index_cache_is_built_in_the_background() {
    let nonce = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let container = std::env::temp_dir().join(format!("pdc-dep-cache-build-{nonce}"));
    let workspace = container.join("workspace");
    let dependency = container.join("dependency");
    fs::create_dir_all(workspace.join("events")).expect("workspace fixture");
    fs::create_dir_all(dependency.join("events")).expect("dependency fixture");
    ...
    .expect("dependency definition");
    let cache_path = container.join("missing/dependency.pdcindex");
    let reference = workspace.join("events/reference.txt");
