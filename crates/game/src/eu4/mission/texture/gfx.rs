...
//! files. This module finds those pairs with the loss-aware `parser` CST
...
fn sprite_type_nodes(parsed: &ParsedFile) -> Vec<parser::CstNode<'_>> {
...
fn is_property(node: &parser::CstNode<'_>) -> bool {
    node.kind() == CstKind::Property
}

fn block_properties(node: parser::CstNode<'_>) -> Vec<parser::CstNode<'_>> {
    node.children()
        .filter(|child| child.kind() == CstKind::Property)
        .collect()
}

fn property_block(prop: parser::CstNode<'_>) -> Option<parser::CstNode<'_>> {
...
fn scalar_key(parsed: &ParsedFile, prop: parser::CstNode<'_>) -> Option<String> {
    prop.children()
...
fn scalar_value(parsed: &ParsedFile, prop: parser::CstNode<'_>) -> Option<String> {
    prop.children()
