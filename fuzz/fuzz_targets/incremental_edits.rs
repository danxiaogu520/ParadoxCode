...
use pdx_syntax::{Eu4FileFormat, SyntaxEdit, parse_eu4};
use pdx_text::TextRange;

fuzz_target!(|data: &[u8]| {
    let Ok(seed) = std::str::from_utf8(data) else { return };
    let mut current = parse_eu4(Eu4FileFormat::PdxScript, seed);
    for (index, chunk) in data.chunks(3).take(16).enumerate() {
        ...
        let full = parse_eu4(Eu4FileFormat::PdxScript, next.source());
