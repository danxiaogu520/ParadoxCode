//! EU4 rules schema and runtime boundary.
//!
...
impl fmt::Debug for Eu4RuleHash {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.debug_tuple("Eu4RuleHash").field(&self.0).finish()
    }
}
...
pub enum ParserKind {
    /// EU4's key/value script grammar.
    PdxScript,
    /// EU4 localisation files.
    Localisation,
...
/// Delimiters used by EU4-style CSV assets.
...
pub enum CwtKeyMatcher {
    ...
    AnyScalar,
    /// Matches a key that declares a dynamic EU4 value set (`value_set[...]`).
    Dynamic(String),
}
...
pub enum CwtValueMatcher {
    ...
    Filepath,
    /// Accepts an EU4 workspace- or scope-derived value set.
    Dynamic(String),
    /// Accepts any non-empty value while defining an EU4 dynamic value set.
    DynamicSet(String),
...
pub struct CwtSemanticRule {
    ...
    /// EU4 scope registers replaced by this rule, represented as `(register, scope)` pairs.
...
/// Normalized logical contents of an EU4 rule database.
#[derive(Clone, Debug, Default, Eq, PartialEq)]
...
/// An immutable EU4 runtime rule set.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Eu4Rules {
    schema_version: u32,
    rule_hash: Eu4RuleHash,
    model: RulesModel,
}

impl Eu4Rules {
    /// Creates an empty EU4 rule set for bootstrapping the crate graph.
    #[must_use]
    pub const fn empty() -> Self {
        Self {
            schema_version: CURRENT_SCHEMA_VERSION,
            rule_hash: Eu4RuleHash::empty(),
            model: RulesModel {
    ...
    /// Returns the bootstrap catalog and its deterministic hash.
    #[must_use]
    pub fn bootstrap() -> Self {
        Self::from_model(RulesModel::bootstrap())
    }
    ...
    #[must_use]
    pub const fn rule_hash(&self) -> Eu4RuleHash {
        self.rule_hash
    }
}

fn canonical_hash(model: &RulesModel) -> Eu4RuleHash {
...
fn write_connection(connection: &mut Connection, rules: &Eu4Rules) -> Result<(), RulesError> {
    schema(connection)?;
...
#[cfg(test)]
mod tests {
    use super::{CURRENT_SCHEMA_VERSION, Eu4Rules, RuleRecord, RulesModel};
    use std::collections::BTreeMap;
    use std::time::{SystemTime, UNIX_EPOCH};

    #[test]
    fn empty_eu4_rules_have_a_stable_schema_identity() {
        let rules = Eu4Rules::empty();
    ...
    fn canonical_hash_is_independent_of_record_insertion_order() {
        let mut first = RulesModel::bootstrap();
        first.records.push(RuleRecord {
            table: "types".to_owned(),
            logical_id: "a".to_owned(),
            source_order: 1,
            fields: BTreeMap::from([(String::from("name"), String::from("event"))]),
        });
        let mut second = RulesModel::bootstrap();
        second.records.push(RuleRecord {
            table: "types".to_owned(),
            logical_id: "a".to_owned(),
            source_order: 1,
            fields: BTreeMap::from([(String::from("name"), String::from("event"))]),
        ...
        assert_eq!(
            Eu4Rules::from_model(first).rule_hash(),
            Eu4Rules::from_model(second).rule_hash()
        );
    }
    ...
    fn sqlite_round_trip_validates_logical_hash() {
        ...
        let path = std::env::temp_dir().join(format!("paradoxcode-eu4-{nonce}.pdxrules"));
        let rules = Eu4Rules::bootstrap();
        rules.write_sqlite(&path).expect("write rules");
        assert_eq!(Eu4Rules::load(&path).expect("load rules"), rules);
