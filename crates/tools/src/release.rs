...
mod tests {
    ...
    fn tar_gz_roundtrips() {
        let archive = create_tar_gz(PAYLOAD, "pdc").expect("create tar.gz");
        assert!(!archive.is_empty());
        // Decompress via file to avoid in-memory streaming issues.
        let path = std::env::temp_dir().join(format!("pdc-rt-{}.tar.gz", std::process::id()));
        ...
        assert_eq!(entries[0].path().expect("path").as_os_str(), "pdc");
        assert_eq!(
    ...
    fn deterministic_zip() {
        let first = create_zip(PAYLOAD, "pdc.exe").expect("create zip");
        let second = create_zip(PAYLOAD, "pdc.exe").expect("create zip again");
    ...
    fn package_and_verify() {
        let nonce = std::time::SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("pdc-release-test-{nonce}"));
    ...
    fn packaging_rejects_oversized_binary() {
        let nonce = std::time::SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("pdc-release-oversized-{nonce}"));
        fs::create_dir_all(&root).expect("temp dir");
    ...
    fn verification_rejects_bad_checksum() {
        let nonce = std::time::SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("pdc-release-bad-checksum-{nonce}"));
        fs::create_dir_all(&root).expect("temp dir");
