...
impl SyntheticVanilla {
    fn create(file_count: usize) -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "engine-vanilla-benchmark-{}-{nonce}",
            std::process::id()
        ));
...
fn dense_file_count() -> usize {
    std::env::var("PDC_BENCH_DENSE_FILES")
        .ok()
        .and_then(|value| value.parse().ok())
        .filter(|count| *count > 0)
...
fn dense_entries() -> usize {
    std::env::var("PDC_BENCH_DENSE_ENTRIES")
        .ok()
        .and_then(|value| value.parse().ok())
        .filter(|count| *count > 0)
...
fn mixed_current_file_count() -> usize {
    std::env::var("PDC_BENCH_MIXED_CURRENT_FILES")
        .ok()
        .and_then(|value| value.parse().ok())
        .filter(|count| *count > 0)
...
fn mixed_current_entries() -> usize {
    std::env::var("PDC_BENCH_MIXED_CURRENT_ENTRIES")
        .ok()
        .and_then(|value| value.parse().ok())
        .filter(|count| *count > 0)
...
fn main() {
    ...
    let cache_path = fixture.root.join("vanilla.pdcindex");
    let (build, cache) =
    ...
    let dense_cache_path = dense_fixture.root.join("vanilla.pdcindex");
