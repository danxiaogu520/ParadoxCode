...
function platformArtifact(): ServerArtifact {
    if (process.platform === 'win32' && process.arch === 'x64') {
        return { target: 'x86_64-pc-windows-msvc', binary: 'paradoxcode.exe', extension: 'zip' };
    }
    if (process.platform === 'linux' && process.arch === 'x64') {
        return { target: 'x86_64-unknown-linux-gnu', binary: 'paradoxcode', extension: 'tar.gz' };
    }
    if (process.platform === 'linux' && process.arch === 'arm64') {
        return { target: 'aarch64-unknown-linux-gnu', binary: 'paradoxcode', extension: 'tar.gz' };
    }
    if (process.platform === 'darwin' && process.arch === 'x64') {
        return { target: 'x86_64-apple-darwin', binary: 'paradoxcode', extension: 'tar.gz' };
    }
...
export function archiveName(version: string, artifact: ServerArtifact): string {
    return `paradoxcode-v${version}-${artifact.target}.${artifact.extension}`;
}
...
export function defaultInstallDirectory(context: vscode.ExtensionContext): string {
    return path.join(context.globalStorageUri.fsPath, 'paradoxcode');
}
