...
impl<'source> Parser<'source> {
                ...
                if self.peek() == Some(b'"') {
                    ...
                    if terminator.is_none() {
                        self.error(
                            ...
                            "a top-level PdxScript value must be assigned to a key",
                        );
                    }
                    ...
                    self.error(
                        ...
                        "unexpected token in PdxScript document",
                    );
    ...
    fn parse_value(&mut self) -> CstNode {
        ...
        let Some(value) = self.parse_bare() else {
            let end = self.position.saturating_add(1).min(self.source.len());
            self.position = end;
            self.error(SyntaxErrorKind::UnexpectedToken, start, end, "expected a PdxScript value");
