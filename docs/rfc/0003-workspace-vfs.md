...
`VanillaIndexCache` 当前 schema 为 `5`。cache 保存 `game_id`、`rule_hash`、source identity/fingerprint、文件元数据、semantic shards、scripted macro 的参数签名和规范化模板 IR、definition/reference 的 UTF-16 位置以及有界的 localisation Hover preview；不保存 Vanilla 源码、CST 或完整 HIR。macro summary 包含定义 kind/name/range、按首次出现排序的参数名和 required 标记，以及可选的 source-independent `MacroTemplate`。模板必须与同一 shard 中的实际 definition identity 一致；SQLite payload 有单项字节、节点与文本预算，加载时严格反序列化并校验所有 range 均位于 owner definition 内。schema 不兼容仍要求重建，不做静默降级。
