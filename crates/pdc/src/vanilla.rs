        ...
        let selected = if let Some(source) = auto_vanilla.source_override.as_ref() {
            ...
            let selection = match select_installation(&report.installations) {
                ...
                None => {
                    ...
                    return Err(format!(
                        "{} (game `{}`) was not found in known installation locations; point the game directory setting at your installation (VS Code: `paradoxcode.gameDirectory`) and reload",
            ...
            Ok((cache, cache_path)) => {
                ...
                if !alternatives.is_empty() {
                    message.push_str(&format!(
                        "; other candidates: {} (override via the game directory setting)",
                        alternatives
                            .iter()
