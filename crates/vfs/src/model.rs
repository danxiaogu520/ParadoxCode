...
impl WorkspaceScanToken {
    ...
    pub fn checkpoint(&self) -> Result<(), WorkspaceError> {
        if self
    ...
    /// Test helper: constructs a token that cancels itself after `checkpoints` checkpoints.
    #[doc(hidden)]
    pub fn cancel_after(checkpoints: usize) -> Self {
        Self {
            cancelled: Arc::new(AtomicBool::new(false)),
    ...
    for node in parsed.root().children() {
        match node.kind() {
            ...
            CstKind::LocalisationEntry => {
                ...
                let decoded = transcode::decode_value(value);
...
/// unsaved and non-file documents while leaving URI/path conversion to `pdc`.
