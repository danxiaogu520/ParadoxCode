...
pub(super) fn decode_range(start: i64, end: i64) -> Result<TextRange, VanillaCacheError> {
...
pub(super) fn decode_position_component(value: i64, label: &str) -> Result<u32, VanillaCacheError> {
...
pub(super) fn decode_file_id(bytes: &[u8]) -> Result<SourceFileId, VanillaCacheError> {
...
pub(super) fn parse_resolution(value: &str) -> Result<FileResolutionPolicy, VanillaCacheError> {
    match value {
...
#[cfg(unix)]
pub(super) fn encode_path(path: &Path) -> Result<(&'static str, Vec<u8>), VanillaCacheError> {
    use std::os::unix::ffi::OsStrExt;
    Ok(("unix-bytes-v1", path.as_os_str().as_bytes().to_vec()))
}

#[cfg(unix)]
pub(super) fn decode_path(bytes: &[u8], encoding: &str) -> Result<PathBuf, VanillaCacheError> {
    use std::ffi::OsStr;
    use std::os::unix::ffi::OsStrExt;
