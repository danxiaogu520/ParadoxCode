...
fn low_position_matrix() {
    ...
    for byte in 0u8..=255 {
        ...
        let text = char::from_u32(code_point)
            ...
            .to_string();
        for set in SETS {
            let encoded = encode_text(&text, set).expect("encode text");
            assert_eq!(
        ...
        let bytes = encode_file(&text, Profile::Script, EscapeSet::Paratranz).expect("encode file");
        assert_eq!(
            decode_file(&bytes, Profile::Script)
                .expect("decode file")
                .text,
            text,
...
fn every_escape_set_member_actually_escapes_in_both_positions() {
    for set in SETS {
        for &byte in set.bytes() {
            let low_char = char::from_u32(0x4E00 + u32::from(byte)).expect("valid code point");
            let low_encoded = encode_text(&low_char.to_string(), set).expect("encode text");
            assert_eq!(
            ...
            if byte >= 0x10 {
                let high_char =
                    char::from_u32((u32::from(byte) << 8) | 0x61).expect("valid code point");
                let high_encoded = encode_text(&high_char.to_string(), set).expect("encode text");
                assert_eq!(
...
fn cp1252_characters_round_trip_in_both_profiles() {
    for (byte, code_point) in CP1252_MAP {
        ...
        if code_point >= 0x1000 {
            let encoded = encode_text(&text, EscapeSet::Paratranz).expect("encode text");
            assert_eq!(
                decode_text(&encoded).text,
                text,
...
fn ascii_and_bom_are_fixed_points() {
    ...
    assert_eq!(
        encode_text(ascii, EscapeSet::Paratranz).expect("encode text"),
        ascii
    );
    ...
    assert_eq!(
        encode_file(pure_ascii, Profile::Script, EscapeSet::Paratranz).expect("encode file"),
        pure_ascii.as_bytes()
    );

    let bom_input = "\u{FEFF}中";
    let bom_encoded = encode_text(bom_input, EscapeSet::Paratranz).expect("encode text");
...
fn dll_full_output_decodes_with_the_canonical_decoder() {
    ...
    for byte in [0x3Au8, 0x3C, 0x3E, 0x3F, 0x7C, 0x2A] {
        ...
        let text = char::from_u32(code_point)
            .expect("valid code point")
            .to_string();
        let dll = encode_text(&text, EscapeSet::DllFull).expect("encode text");
        let canonical = encode_text(&text, EscapeSet::Paratranz).expect("encode text");
...
fn classification_survives_a_full_round_trip() {
    ...
    let encoded = encode_text(readable, EscapeSet::Paratranz).expect("encode text");
