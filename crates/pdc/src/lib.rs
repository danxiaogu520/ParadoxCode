//! Minimal JSON-RPC/LSP runtime for the ParadoxCode language server (`pdc`).
//!
...
/// Maximum number of closed Project files that one workspace validation pass publishes.
...
/// to an explicit `pdc/reindexWorkspace`. Each attempt restarts from a fresh
...
pub(crate) const WATCHED_FILES_REGISTRATION_ID: &str = "pdc-source-roots";
pub(crate) const WATCHED_FILES_REQUEST_ID: &str = "pdc/register-source-root-watchers";
...
#[cfg(test)]
pub(crate) use engine::DocumentId;
#[cfg(test)]
