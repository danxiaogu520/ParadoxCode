...
状态：`implemented, acceptance reopened`（2026-07-20）；查询功能和 LSP integration 已实现，query-time 全 workspace 重解析、深拷贝 snapshot，以及 event-loop 上的 parse/lower、diagnostics 和语言查询已消除；analysis 内部细粒度取消与 workspace scan 取消仍不满足已接受边界。
