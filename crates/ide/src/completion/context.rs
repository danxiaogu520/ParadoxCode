...
use hir::HirFile;
use rules::{ProfileRootEntryInsertion, RuleShape};
use text::{LogicalPath, TextSize};
...
fn seed_dynamic_body_scope(
    ...
    let descriptor = snapshot
        ...
        .get(kind.as_str());
    if !descriptor
        .is_some_and(|descriptor| hir::semantic_type_path_matches(descriptor, logical_path))
    {
        return;
    }
    ...
    if let [single] = scopes.as_slice() {
        scope.current = engine::intern_shard_string(single);
    }
}
...
fn semantic_root_entry_context(
    ...
    snapshot
        ...
        .filter_map(|(type_name, descriptor)| {
            ...
            if !hir::semantic_type_path_matches(descriptor, Some(logical_path)) {
                return None;
            }
...
) -> Vec<(&'rule rules::SemanticRule, Vec<std::sync::Arc<str>>)> {
    let mut out = Vec::new();
...
pub(crate) fn completion_structural_containers(
    ...
    if !transparent_wrapper {
        structural_path.push(engine::intern_shard_string(property_key));
    }
