...
export class LspClient {
  constructor(child, { timeoutMs = 30_000 } = {}) {
    ...
    child.stdout.on('end', () => this.failWaiters(new LspProtocolError('server closed stdout')));
    child.on('error', (error) => this.failWaiters(error));
    child.on('close', (code, signal) => {
      this.closed = true;
      if (code !== 0 || signal) {
        this.failWaiters(
          new LspProtocolError(`server exited with code ${code ?? 'unknown'}${signal ? ` (${signal})` : ''}`),
        );
      } else {
        this.failWaiters(new LspProtocolError('server exited before the expected response'));
      }
    });
  ...
  consume(chunk) {
    ...
    while (true) {
      ...
      try {
        ...
        this.protocolError = new LspProtocolError(`invalid JSON from server: ${error.message}`);
        this.failWaiters(this.protocolError);
        return;
      }
  ...
  async next(timeoutMs) {
    ...
    if (this.closed) throw new LspProtocolError('the server is no longer running');
    return new Promise((resolveMessage, reject) => {
      const timer = setTimeout(() => {
        const index = this.waiters.findIndex((candidate) => candidate.resolve === resolveMessage);
        if (index >= 0) this.waiters.splice(index, 1);
        reject(new LspProtocolError(`timed out after ${timeoutMs} ms waiting for the server`));
      }, timeoutMs);
  ...
  send(message) {
    if (this.closed || this.child.stdin.destroyed) {
      throw new LspProtocolError('cannot write to stopped server');
    }
