...
//! mirrors what a static [`rules::SemanticRule`] provides for a command:
...
use engine::{AnalysisSnapshot, CacheDomain, DocumentSource, DynamicParameterSignature};
...
impl DynamicScopeTransition {
    fn from_rule(rule: &rules::SemanticRule) -> Self {
        Self {
...
fn dynamic_rule_report(
    ...
    if std::env::var("PDC_DEBUG_DYNAMIC_RULES").is_ok_and(|value| !value.is_empty()) {
...
impl<'a> Derivation<'a> {
    ...
    fn walk_property(
        ...
        let matching: Vec<&rules::SemanticRule> =
...
impl<'a> SiteDerivation<'a> {
    ...
    fn record_scalar_sites(
        ...
        let leaf: Vec<&rules::SemanticRule> = matching
            .iter()
            .copied()
            .filter(|rule| matches!(rule.shape, RuleShape::Leaf | RuleShape::LeafValue))
            .collect();
        let quoted_payload: Vec<&rules::SemanticRule> = matching
            .iter()
            .copied()
...
impl SiteDestinations {
    fn push(&mut self, context: Arc<str>, path: Vec<Arc<str>>, rule: &rules::SemanticRule) {
        let transition = DynamicScopeTransition::from_rule(rule);
...
fn rule_operator_matches(rule: &rules::SemanticRule, operator: Option<&str>) -> bool {
    rule.operator
        .as_deref()
...
fn required_scopes_of_rows(rows: &[&rules::SemanticRule]) -> Vec<String> {
