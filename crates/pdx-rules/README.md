...
- `CURRENT_SCHEMA_VERSION`：当前 runtime SQLite schema，值为 `18`。
...
RFC 0013 规定 `rules/eu4/` 是静态 EU4 规则的唯一 source authority。当前 source format 为 `7`，固定输入包括 `manifest.json`、`catalog.json`、`semantic-rules.json`、`enum-values.json`、`type-root-keys.json`、`type-root-scopes.json`、`type-descriptors.json` 和 `localisation-bindings.json`。
...
- 当前 runtime schema 为 `18`，不接受其他 schema；规则身份由 `game_id` 和 canonical `rule_hash` 校验。
