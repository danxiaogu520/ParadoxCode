...
pdx-rules -> pdx-bake
pdx-rules + pdx-game -> pdx-hir / pdx-engine / pdx-analysis
```
...
| `pdx-bake` | 严格读取第一方 JSON 规则源码、校验并生成 artifact/manifest | CWT 输入、runtime 依赖、网络同步、用户规则覆盖 |
...
- `pdx-bake`：严格 source schema、stable identity、invariant、deterministic hash、artifact round-trip；
- `pdx-engine`：scope transition、unknown context、typed lowering、root order、overlay、覆盖解析、shard replacement、snapshot；
